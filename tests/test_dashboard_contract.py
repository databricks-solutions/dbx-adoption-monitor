...
def test_dashboard_contract_matches_dashboard(repo_root: Path) -> None:
    ...
    # Canary widgets from the current canonical dashboard. Titles are stored as
    # {"value": ...} objects in the current Lakeview format.
    assert '"value": "Costliest Questions"' in dashboard_text
    assert '"value": "Serving cost by model"' in dashboard_text
...
def test_widget_encodings_reference_bound_fields(repo_root: Path) -> None:
    ...
    for page in dashboard["pages"]:
        for layout_item in page.get("layout", []):
            ...
            # Filter widgets (single/multi-select, date-range, range-slider) bind
            # the field they filter via `queryName`, not their own query outputs,
            # so the encoding-binding invariant only applies to visualizations.
            widget_type = widget.get("spec", {}).get("widgetType", "")
            if widget_type.startswith("filter-") or widget_type == "range-slider":
                continue
            # Collect every field name defined by the widget's queries. The
            # current Lakeview format does not always carry `datasetName` on the
            # node holding `fields`, so gather from any node exposing a fields list.
            bound_fields = {
                field["name"]
...
def test_vector_search_page_contract(repo_root: Path) -> None:
    ...
    assert len(vector_page["layout"]) == 10
